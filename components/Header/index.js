...
export default function Header({ children }) {
    ...
    const resume = "/CV/resume.pdf"; // Define the path to your resume
    ...
    async function getReposCount() {
        ...
        if (localStorage.getItem("repo_counts") === null) {
            try {
                ...
                console.error("Failed to fetch repos count:", error);
            }
        } else {
            ...
            <Container>
                ...
                <div className={`w-full h-[70vmin] flex align-center items-center justify-center flex-row p-[20px] flex-wrap mt-[50px]`}>
                    <div className={`w-full h-full mb-[50px] relative md:w-[50%]`}>
                        ...
                        {resumeActive && <ResumeViewer openResume={openResume} resume={resume} />}
                    </div>
                    <div data-aos="fade-left" className={`main w-full h-auto hidden md:block md:w-[50%] relative`}>
                        ...
                        <div data-aos="fade-up" className={`circleA`}>
                            <img src={languages.languages.length > 0 ? languages.languages[0] : "https://cdn.jsdelivr.net/gh/devicons/devicon/icons/python/python-original.svg"} className={`langImgA`} />
                        </div>
                        <div data-aos="fade-right" className={`circleB`}>
                            <img src={languages.languages.length > 1 ? languages.languages[1] : "https://cdn.jsdelivr.net/gh/devicons/devicon/icons/python/python-original.svg"} className={`langImgB`} />
                        </div>
                        <div data-aos="fade-left" className={`circleC`}>
                            <img src={languages.languages.length > 2 ? languages.languages[2] : "https://cdn.jsdelivr.net/gh/devicons/devicon/icons/python/python-original.svg"} className={`langImgC`} />
                        </div>
                    </div>
                </div>
            </Container>
...
function ResumeViewer({ openResume, resume }) {
    function downloadCv() {
        let link = document.createElement("a");
        link.href = resume; // Use the passed resume URL
    ...
    return (
        <div className="fixed top-0 left-0 w-screen h-screen bg-dark-400 z-[1500] flex flex-row items-center justify-center">
            <div id="box" className="w-[100%] h-[99%] mx-auto bg-dark-100 overflow-hidden rounded-md md:w-[70%]">
                ...
                <iframe src={"/CV/resume.pdf"} style={{ border: "none" }} className="w-full h-full overflow-scroll bg-white-200 mt-0"></iframe>
                <br />
                <br />
                <br />
            </div>
